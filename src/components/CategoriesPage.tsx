...
export default function CategoriesPage() {
  ...
  const categoryMatchesSearch = (category: Category, searchTerm: string): boolean => {
    if (!searchTerm) return true;
    
    const searchLower = searchTerm.toLowerCase();
    const matchesName = category.categoryName?.toLowerCase().includes(searchLower);
    const matchesShort = category.shortDescription?.toLowerCase().includes(searchLower);
    const matchesLong = category.longDescription?.toLowerCase().includes(searchLower);
  ...
  const filterCategoryTree = (
    ...
    return categories.reduce((filtered, category) => {
      ...
      const filteredChildren = filterCategoryTree(category.children, searchTerm, filterType);
      
      // Include category if it matches or has matching children
      const shouldInclude = (matchesSearch && matchesFilter) || filteredChildren.length > 0;
  ...
  const filteredCategoryTree = filterCategoryTree(categoryTree, searchTerm, filterType);

  // Count categories recursively
  const countCategoriesInTree = (tree: (Category & { children: Category[] })[]): number => {
  ...
  const CategoryTreeItem = ({ 
    ...
    const borderColor = level === 0 ? 'border-blue-300' : 
                       level === 1 ? 'border-orange-300' : 
                       'border-purple-300';

    return (
      <div key={category.id}>
        {/* Category Row */}
        <div className={`hover:bg-gray-50 transition-colors ${level > 0 ? 'bg-gray-25' : ''}`}>
          <div className={`grid grid-cols-12 gap-4 px-6 py-4 items-center ${level > 0 ? `border-l-4 ${borderColor} ${indentClass}` : ''}`}>
            ...
            <div className="col-span-4">
              <div className="flex items-center space-x-3">
                <div className={`${level > 0 ? 'w-10 h-10' : 'w-12 h-12'} rounded-lg overflow-hidden border border-gray-200 bg-gray-50 flex items-center justify-center`}>
                  <img
                ...
                <div>
                  <h4 className={`font-semibold text-gray-800 ${level > 0 ? 'text-sm' : ''}`}>
            ...
            <div className="col-span-3">
              <p className={`text-gray-600 line-clamp-2 ${level > 0 ? 'text-xs' : 'text-sm'}`}>
        ...
        {isExpanded && hasChildren && (
          <div className={level === 0 ? 'bg-gray-50' : level === 1 ? 'bg-gray-100' : 'bg-gray-150'}>
